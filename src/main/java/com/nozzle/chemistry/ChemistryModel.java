...
public class ChemistryModel {
    ...
    public record SpeciesData(
        ...
        public double enthalpy(double temperature) {
            double[] a = temperature < 1000 ? lowTempCoeffs : highTempCoeffs;
            double T = temperature;
            double hOverRT = a[0] + a[1]*T/2 + a[2]*T*T/3 + a[3]*T*T*T/4 + a[4]*T*T*T*T/5 + a[5]/T;
            return R_UNIVERSAL * T * hOverRT / molecularWeight;
        }
        ...
        public double entropy(double temperature) {
            double[] a = temperature < 1000 ? lowTempCoeffs : highTempCoeffs;
            double T = temperature;
            double sOverR = a[0]*Math.log(T) + a[1]*T + a[2]*T*T/2 + a[3]*T*T*T/3 + a[4]*T*T*T*T/4 + a[6];
         ...
         */
        public double gibbsOverRT(double temperature) {
            double[] a = temperature < 1000 ? lowTempCoeffs : highTempCoeffs;
            double T = temperature;
            double hOverRT = a[0] + a[1]*T/2 + a[2]*T*T/3 + a[3]*T*T*T/4 + a[4]*T*T*T*T/5 + a[5]/T;
            double sOverR = a[0]*Math.log(T) + a[1]*T + a[2]*T*T/2 + a[3]*T*T*T/3 + a[4]*T*T*T*T/4 + a[6];
