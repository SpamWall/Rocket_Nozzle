...
### Throat Circular Arc (Downstream)

Before the initial data line can be placed, the geometry locates the end of the
downstream circular-arc throat transition. The arc has radius
...
## 11. Convergent Section (ConvergentSection)

### Geometry

The convergent-section wall is generated by `ConvergentSection` from three
C¹-continuous segments, all referenced to `x = 0` at the throat (positive x
downstream):

**1. Upstream circular arc** — radius `r_cu = upstreamCurvatureRatio × r_t`,
centred at `(0, r_t + r_cu)`, sweeping from the throat `(0, r_t)` upstream to
the tangent angle `θ_c = convergentHalfAngle`:
...
The arc tangent at `α = θ_c` is `dy/dx = −tan(θ_c)`, which is exactly the
slope of the convergent cone — guaranteeing C¹ continuity at the transition.

**2. Conical section** — straight cone at half-angle `θ_c`, running upstream
from `(x_arc, y_arc)` until the radius reaches the chamber radius
...
**3. Chamber face** — the single upstream-boundary point `(x_cone, r_c)`.

The complete contour runs from `x_cone` (negative, upstream) to just upstream of
`x = 0`, ordered from chamber to throat so that prepending to a divergent
`NozzleContour` via `withConvergentSection()` produces a monotone x sequence.

### Sonic-Line Discharge Coefficient
...
`BoundaryLayerCorrection` starts its integration at the first point of the
supplied `NozzleContour`.  When `NozzleContour.withConvergentSection()` is
used to build a full-geometry contour, the integration automatically begins at
the chamber face.  For wall points at `x < 0` where no MOC flow data is
available, the local Mach number is obtained from the isentropic area–velocity
relation by bisection on `A/A*(M)` evaluated at the local wall radius, correctly
giving subsonic values throughout the convergent section.

### Geometry-Complete Exports

Because all exporters (`DXFExporter`, `STLExporter`, `STEPExporter`,
`CFDMeshExporter`, `OpenFOAMExporter`) consume a `NozzleContour`, passing the
output of `withConvergentSection()` to any exporter automatically produces a
complete nozzle profile from the chamber face to the exit plane — no exporter
